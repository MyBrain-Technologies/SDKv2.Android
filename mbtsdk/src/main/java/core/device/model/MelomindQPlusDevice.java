...
/**
 * TODO: this is just a copy of {@link MelomindDevice}, this class should present a config of Melomind Q Plus device.
 */
@Keep
public class MelomindQPlusDevice extends MbtDevice{

    public MelomindQPlusDevice(@NonNull final String address, final String name){
        super(address, name, MbtDeviceType.MELOMIND_Q_PLUS, MbtFeatures.MELOMIND_QPLUS_NB_CHANNELS);
    ...
    public MelomindQPlusDevice(@NonNull final BluetoothDevice device){
        super(device, MbtDeviceType.MELOMIND_Q_PLUS, MbtFeatures.MELOMIND_QPLUS_NB_CHANNELS);
        this.acquisitionLocations = Arrays.asList(
                MbtAcquisitionLocations.P3,
                MbtAcquisitionLocations.P4,
                MbtAcquisitionLocations.F3,
    ...
    public static InternalConfig convertRawInternalConfig(Byte[] rawConfig) {
        ...
        if(rawConfig.length < SAMP_RATE_INDEX ) {
            Log.e(MelomindQPlusDevice.class.getSimpleName(), "Invalid internal config size : "+Arrays.toString(rawConfig)+". Minimum expected size is "+SAMP_RATE_INDEX );
            return null;
        }
        ...
        return new InternalConfig(
                MbtFeatures.MELOMIND_QPLUS_NB_CHANNELS,
                rawConfig[0],
                rawConfig[1],
                rawConfig[2],
                rawConfig[3],
