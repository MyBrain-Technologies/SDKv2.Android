...
public class MbtRecordBuffering {
    ...
    public boolean storeRecordBuffer(@NonNull MbtDevice device, @NonNull RecordConfig recordConfig) {
        ...
        if(recordConfig.getDuration() != UNDEFINED_DURATION && eegPacketsBuffer.size() > recordConfig.getDuration())
            eegPacketsBuffer = new ArrayList<>(eegPacketsBuffer.subList(0,recordConfig.getDuration()));
    ...
    public boolean isEegPacketsBufferEmpty() {
        return eegPacketsBuffer.isEmpty();
    }
}
