 ...
 */
class MbtBluetoothManagerkt(context: Context) : BaseModuleManager(context) {
    private var bluetoothContext: BluetoothContext? = null
    private var bluetoothInitializer: BluetoothInitializer? = null
    private var bluetoothForDataStreaming: MbtDataBluetooth? = null
    ...
    private val MAX_CONNECTION_RETRY = 2
    ...
    private fun onBluetoothResponseReceived(response: Any, command: BluetoothCommand<*, *>) {
        if (command is Mtu) onMtuChanged(response === command.data)
    }
    ...
    inner class RequestThread(name: String?, priority: Int) : HandlerThread(name, priority) {
        ...
        private fun initBluetoothParameters(event: StartOrContinueConnectionRequestEvent) {
            ...
            if (event.isClientUserRequest && bluetoothForDataStreaming == null) {
                if ((bluetoothContext!!.deviceTypeRequested == MbtDeviceType.MELOMIND)) {
                    bluetoothForDataStreaming = MbtBluetoothLE(mContext, this@MbtBluetoothManagerkt)
                    if (bluetoothContext!!.connectAudioIfDeviceCompatible) bluetoothForAudioStreaming = MbtBluetoothA2DP(mContext, this@MbtBluetoothManagerkt)
                } else if ((bluetoothContext!!.deviceTypeRequested == MbtDeviceType.VPRO)) bluetoothForDataStreaming = MbtBluetoothSPP(mContext, this@MbtBluetoothManagerkt)
            }
        }
        ...
        private fun startOrContinueConnectionOperation(isClientUserRequest: Boolean) {
            ...
            if (!isConnectionInterrupted) {
                ...
                when (currentState) {
                    BluetoothState.IDLE, BluetoothState.DATA_BT_DISCONNECTED -> readyForBluetoothOperation
...
// assert that headset is not already connected//when the BUS has returned the device object//Request sent to the BUS in order to get device from the device manager : the BUS should return a null object if it's the first connection, or return a non null object if the user requests connection whereas a headset is already connected
    ...
    private fun onMtuChanged(isSuccess: Boolean) {
    ...
    fun disconnectAllBluetooth(disconnectAudioIfConnected: Boolean) {
        ...
        disconnect(bluetoothContext!!.deviceTypeRequested.protocol)
    }
            ...
            BluetoothState.DEVICE_FOUND -> {
                ...
                if (bluetoothContext!!.connectAudioIfDeviceCompatible) {
                    if (bluetoothForAudioStreaming == null) {
                        bluetoothForAudioStreaming = MbtBluetoothA2DP(mContext, this@MbtBluetoothManagerkt)
    ...
    fun requestCurrentConnectedDevice(callback: SimpleRequestCallback<MbtDevice?>) {
        MbtEventBus.postEvent(GetDeviceEvent(), MbtEventBus.Callback<PostDeviceEvent?> { deviceEvent ->
            MbtEventBus.registerOrUnregister(false, this)
            callback.onRequestComplete(deviceEvent.device)
    ...
    private fun updateConnectionState(state: BluetoothState?) {
        if ((state != null) && !state.isAudioState && (bluetoothContext != null) && (!isConnectionInterrupted || (state == BluetoothState.CONNECTION_INTERRUPTED))) {
    ...
    private val isDataBluetoothConnected: Boolean
        private get() = bluetoothForDataStreaming!!.isConnected
    ...
    private val isAudioBluetoothConnected: Boolean
        private get() = bluetoothForAudioStreaming != null && bluetoothForAudioStreaming!!.isConnected
    ...
    val isConnected: Boolean
        get() = (if (bluetoothContext!!.connectAudioIfDeviceCompatible) (isDataBluetoothConnected && isAudioBluetoothConnected) else isDataBluetoothConnected)

    /**
     * Gets current state according to bluetooth protocol value
     */
    private val currentState: BluetoothState
        private get() = bluetoothForDataStreaming!!.currentState

    val deviceNameRequested: String?
        get() = bluetoothContext!!.deviceNameRequested

    private val currentDevice: BluetoothDevice
        private get() = bluetoothForDataStreaming!!.currentDevice
     ...
     */
    private fun notifyEventReceived(eventIdentifier: DeviceCommandEvent?, eventData: Any?) {
        MbtEventBus.postEvent(BluetoothResponseEvent(eventIdentifier, eventData))
    }

    companion object {
        private val TAG = MbtBluetoothManagerkt::class.java.simpleName
    }
