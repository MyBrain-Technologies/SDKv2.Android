...
public abstract class MbtBluetooth implements BluetoothInterfaces.IConnect, BluetoothInterfaces.IStream{ //todo [code review] refactor interfaces > abstract classes : see streamstate comments to understand

    private final static String TAG = MbtBluetooth.class.getName();

    @NonNull
    private StreamState streamState = StreamState.IDLE; //todo [code review] streamState variable can be inherited from Stream abstract class instead of IStream interface that just implements methods

    private volatile BtState currentState = BtState.IDLE; //todo [code review] add @NonNull annotation + rename into bluetoothState

    protected boolean isUpdating; //todo [code review] remove useless
    ...
    public void notifyConnectionStateChanged(@NonNull BtState newState) {
        if(!newState.equals(currentState)){
            ...
            }if(currentState.isDisconnectableState())  //if a failure occurred //todo [code review]  check if a "else" is not missing here
    ...
    protected void notifyBatteryReceived(int value){
        mbtBluetoothManager.notifyDeviceInfoReceived(DeviceInfo.BATTERY, String.valueOf(value));//todo [code review] keep battery value as integer ?
    }

    void notifyHeadsetStatusEvent(byte code, int value){ //todo [code review]  only available in Melomind to this day > see if vpro will need it
...
//        }
    ...
    public void notifyNewDataAcquired(@NonNull final byte[] data) {//todo [code review] call this method to notify the device manager (and all the managers in general) when the bluetooth receives data from the headset (saturation & offset for example instead of notifyNewHeadsetStatus located in the MbtBluetoothLe)
    ...
    public BtState getCurrentState() { return currentState; }//todo [code review] rename getBluetoothState

    void setCurrentState(BtState currentState) {//todo [code review] rename setBluetoothState
     ...
     */
    protected Object waitResponseForCommand(int timeout){ //todo [code review] rename startWait/wait
    ...
    public void notifyCommandResponseReceived(Object response) { //todo [code review] rename stopWait
    ...
    public boolean isStreaming() {
        return streamState == StreamState.STARTED; //todo [code review] add OR == StreamState.STREAMING
    }
