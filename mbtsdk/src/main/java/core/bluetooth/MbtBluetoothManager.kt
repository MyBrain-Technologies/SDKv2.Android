 ...
 */
open class MbtBluetoothManager(context: Context) : BaseModuleManager(context) {
  class MbtBluetoothManagerWorking(context: Context) : MbtBluetoothManager(context) {
    ...
    inner class RequestThread(name: String?, priority: Int) : HandlerThread(name, priority) {
      ...
      fun parseRequest(request: BluetoothRequests) {
        ...
        when (request) {
          ...
          is ReadRequestEvent -> startReadOperation(request.deviceInfo)
          is DisconnectRequestEvent -> {
            if (request.isInterrupted) cancelPendingConnection(request.isInterrupted)
            else disconnectAllBluetooth(true)
          }
          is StreamRequestEvent -> {
            when {
              request.isStartStream -> startStreamOperation(request.monitorDeviceStatus())
              request.isStopStream -> stopStreamOperation()
              else -> isRequestProcessing(false)
            }
          }
          is CommandRequestEvent -> bluetoothForDataStreaming.sendCommand(request.command)
        }
      }

      private fun initBluetoothParameters(event: StartOrContinueConnectionRequestEvent) {
        bluetoothInitializer = BluetoothInitializer()
        bluetoothContext = BluetoothContext(mContext,
        ...
        if (event.isClientUserRequest && !::bluetoothForDataStreaming.isInitialized) {
          if ((bluetoothContext.deviceTypeRequested == MELOMIND)) {
            bluetoothForDataStreaming = MbtBluetoothLE(mContext, this@MbtBluetoothManagerWorking)
            if (bluetoothContext.connectAudioIfDeviceCompatible) bluetoothForAudioStreaming = MbtBluetoothA2DP(mContext, this@MbtBluetoothManagerWorking)
          } else if ((bluetoothContext.deviceTypeRequested == VPRO)) bluetoothForDataStreaming = MbtBluetoothSPP(mContext, this@MbtBluetoothManagerWorking)
        }
      }
    ...
    private fun notifyDeviceFound(newState: BluetoothState) {
      ...
      if (bluetoothContext.connectAudioIfDeviceCompatible) {
        if (bluetoothForAudioStreaming == null) {
          bluetoothForAudioStreaming = MbtBluetoothA2DP(mContext, this@MbtBluetoothManagerWorking)
    ...
    fun notifyResponseReceived(response: Any?, command: MbtCommand<*>?) {
      ...
      if (command is DeviceCommand<*, *>) notifyDeviceResponseReceived(response, command)
      else if (command is BluetoothCommand<*, *>) onBluetoothResponseReceived(response, command)
      isRequestProcessing(false)
    }
