...
enum class BluetoothState {
   ...
   */
  READING_FIRMWARE_VERSION, READING_FIRMWARE_VERSION_SUCCESS, READING_HARDWARE_VERSION, READING_HARDWARE_VERSION_SUCCESS, READING_SERIAL_NUMBER, READING_SERIAL_NUMBER_SUCCESS, READING_MODEL_NUMBER,
   ...
   */
  BONDED, CHANGING_BT_PARAMETERS, BT_PARAMETERS_CHANGED,
   ...
   */
  CONNECTED_AND_READY, AUDIO_BT_CONNECTION_SUCCESS, AUDIO_BT_DISCONNECTED,  /// FROM THIS STATE, THE CONNECTION PROCESS IS CONSIDERED COMPLETED : THE FOLLOWING STATES DEALS WITH OTHERS BLUETOOTH OPERATIONS
   ...
   *
   * The device does not have a Bluetooth interface or does not support Bluetooth Low Ebergy
   *
   */
   ...
   */
  var associatedError: BaseError? = null
    private set
   ...
   */
  fun isResettableState(previousState: BluetoothState): Boolean {
    return this == DATA_BT_DISCONNECTED || isDisconnectableState() && previousState.ordinal < ordinal
  }

  /**
   * A disconnection need to be performed if the connection process failed while the headset was connected
   */
  fun isDisconnectableState(): Boolean {
    return isAFailureState()
  }

  fun isReadingDeviceInfoState(): Boolean {
    return this == READING_FIRMWARE_VERSION || this == READING_HARDWARE_VERSION || this == READING_SERIAL_NUMBER || this == READING_MODEL_NUMBER
  }

  fun isConnectionInProgress(): Boolean {
    return getOrder() < CONNECTED_AND_READY.ordinal
  }
  ...
  fun isAudioState(): Boolean {
    return this == AUDIO_BT_CONNECTION_SUCCESS || this == AUDIO_BT_DISCONNECTED
  }

  fun notEquals(vararg states: BluetoothState): Boolean {
    for (state in states) {
