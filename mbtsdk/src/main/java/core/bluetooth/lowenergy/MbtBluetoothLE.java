   ...
   boolean checkServiceAndCharacteristicValidity(@NonNull UUID service, @NonNull UUID characteristic){
    ...
    void notifyConnectionResponseReceived(byte mailboxEvent, byte mailboxResponse) {
        ...
        if(mailboxEvent == DeviceCommandEvents.MBX_CONNECT_IN_A2DP){
            if(BitUtils.isEquals(CMD_CODE_CONNECT_IN_A2DP_JACK_CONNECTED, mailboxResponse))
                mbtBluetoothManager.notifyConnectionStateChanged(BtState.JACK_CABLE_CONNECTED);

            else if(BitUtils.isEquals(CMD_CODE_CONNECT_IN_A2DP_SUCCESS, mailboxResponse)
                || BitUtils.isEquals(CMD_CODE_CONNECT_IN_A2DP_FAILED_ALREADY_CONNECTED, mailboxResponse))
