...
final class MbtGattController extends BluetoothGattCallback {
    ...
    MbtDeviceAcquisition mbtDeviceAcquisition;

    public MbtGattController(Context context, MbtBluetooth bluetoothController) {
        super();
        this.bluetoothController = bluetoothController;
        this.mbtDeviceAcquisition = new MbtDeviceAcquisition();
    ...
    public void onCharacteristicRead(BluetoothGatt gatt, BluetoothGattCharacteristic characteristic, int status) {
        ...
        if (characteristic.getUuid().compareTo(MelomindCharacteristics.CHARAC_INFO_FIRMWARE_VERSION) == 0) {
            bluetoothController.getMelomindDevice().setFirmwareVersion(new String(characteristic.getValue())) ;
        }
        if (characteristic.getUuid().compareTo(MelomindCharacteristics.CHARAC_INFO_HARDWARE_VERSION) == 0) {
            bluetoothController.getMelomindDevice().setHardwareVersion(new String(characteristic.getValue()));
        }
        if (characteristic.getUuid().compareTo(MelomindCharacteristics.CHARAC_INFO_SERIAL_NUMBER) == 0) {
        ...
        if (characteristic.getUuid().compareTo(MelomindCharacteristics.CHARAC_MEASUREMENT_BATTERY_LEVEL) == 0) {
    ...
    public void readBattery() {
        ...
        if (level != mbtDeviceAcquisition.getLastKnownBatteryLevel()) {
            uiAccess.post(new Runnable() {
                public final void run() {
                    bluetoothController.notifyBatteryLevelChanged(level);
                }
            });
            this.mbtDeviceAcquisition.setBatteryLevel(level);
        }
    }
