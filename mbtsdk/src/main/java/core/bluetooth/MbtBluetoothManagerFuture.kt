 ...
 */
class MbtBluetoothManagerFuture(context: Context) : MbtBluetoothManager(context) {
  ...
  companion object {
    private val TAG = MbtBluetoothManagerFuture::class.java.simpleName
    ...
    private fun initBluetoothParameters(event: StartOrContinueConnectionRequestEvent) {
      ...
      if (event.isClientUserRequest && !::bluetoothForDataStreaming.isInitialized) {
        if ((bluetoothContext.deviceTypeRequested == MELOMIND)) {
          bluetoothForDataStreaming = MbtBluetoothLE(mContext, this@MbtBluetoothManagerFuture)
          if (bluetoothContext.connectAudioIfDeviceCompatible) bluetoothForAudioStreaming = MbtBluetoothA2DP(mContext, this@MbtBluetoothManagerFuture)
        } else if ((bluetoothContext.deviceTypeRequested == VPRO)) bluetoothForDataStreaming = MbtBluetoothSPP(mContext, this@MbtBluetoothManagerFuture)
      }
    }
  ...
  private fun notifyDeviceFound(newState: BluetoothState) {
    ...
    if (bluetoothContext.connectAudioIfDeviceCompatible) {
      if (bluetoothForAudioStreaming == null) {
        bluetoothForAudioStreaming = MbtBluetoothA2DP(mContext, this@MbtBluetoothManagerFuture)
