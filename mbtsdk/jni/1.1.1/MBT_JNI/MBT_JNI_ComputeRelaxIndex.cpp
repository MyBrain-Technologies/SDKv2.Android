...
Java_mybraintech_com_mbtsdk_core_signalprocessing_MBTComputeRelaxIndex_nativeComputeRelaxIndexNew(JNIEnv *env,
    ...
    std::vector<float>calibParams = calibrationParameters["SNRCalib_ofBestChannel"];
    ...
    //int nbPacket = signalMatrix.size().second/(samprate)-3;
    ...
    float tmp_NormalizedRelaxIndex = MBT_NormalizeRelaxIndex(tmp_SmoothedRelaxIndex, calibParams);
