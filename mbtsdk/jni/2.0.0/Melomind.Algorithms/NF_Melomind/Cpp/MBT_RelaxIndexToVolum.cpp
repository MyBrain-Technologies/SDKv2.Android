...
float MBT_RelaxIndexToVolum(const float smoothedRelaxIndex, std::map<std::string, std::vector<float> > parametersFromCalibration)
{
    float volum;
    ...
    {
        std::vector<float> tmp_SNRCalib = parametersFromCalibration["SNRCalib_ofBestChannel"];
        vector<double> Copytmp_SNRCalib(tmp_SNRCalib.begin(), tmp_SNRCalib.end()); // convert vector of float in vector of double
        std::vector<double> quants = Quantile(Copytmp_SNRCalib);
