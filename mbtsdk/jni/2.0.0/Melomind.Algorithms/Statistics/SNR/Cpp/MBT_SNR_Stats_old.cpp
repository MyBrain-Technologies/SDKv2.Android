...
int SNR_Statistics::ExerciseSwitch(vector<float> SNR)
{
    ...
    // sort "rows"
	vector<long unsigned> indexes = sort_indexes(tmp1);
...
pair<float,float> SNR_Statistics::ExerciseAreaUpAndPerformance(vector<float> SNR, double ExerciseThreshold)
{
	...
	vector<double> Xcross_down, Ycross_down, Xcross_up, Ycross_up, tmp_All_x, tmp_All_y, AreaUp, KeepXcross_up, TimeLength;
	...
	if ( (!KeepXcross_up.empty()) && (!Xcross_down.empty()) )
	{
	...
	// IMPLEMENTATION OF AREA UP -- NEEDS TO BE TESTED
	// double val = 0;
	...
	vector<long unsigned> Test_Indices = sort_indexes(Xcross_up);
	sort(Xcross_up.begin(), Xcross_up.end());
...
map<string, float> SNR_Statistics::CalculateSNRStatistics(vector<float> SNR, float ExerciseThreshold)
{
	...
	OverallArea = (float) trapz(seq, ThresholdVec);
